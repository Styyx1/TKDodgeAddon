                   ...
                   REX::TOML::Setting<T> &setting, const char *help)
{
    ...
    const bool changed = ImGuiMCP::ImGui::SliderScalar(label, ImGuiMCP::ImGuiDataType_::ImGuiDataType_Float, &slider_var,
                                                 &min, &max, fmt);
    if (changed)
    {
...
{
    const bool changed = ImGuiMCP::ImGui::Checkbox(label, &value_var);
    if (changed)
    {
